...
public class RegisterDao {
	...
	public static List<RegisteredCustomers> getAllRecords(){  
	 ...
	 public int updatedetails(String name, String address,String dob,String email,String nic,int tel1,String gender){
		 int status = 0;
		 try{
			//get database connection
         	Connection con = DBConnectionUtil.getConnection();
         	//SQL statement
            PreparedStatement prst = con.prepareStatement("UPDATE registered SET name = ?, address = ?, phone = ?, dob = ?, gender = ?, nic =?  WHERE email = ? "); 
		    prst.setString(1, name);
		    prst.setString(2, address);
		    prst.setInt(3,tel1);
		    prst.setString(4, dob);
		    prst.setString(5, gender);
		    prst.setString(6, nic);
